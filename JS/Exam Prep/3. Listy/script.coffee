Solve = (args) ->
  ...
  evaluate = (operation) ->
    for name, value of variables
      while operation.indexOf(name + ' ') isnt -1
    ...
    if params.trim() is "" then return ""
    params = params.trim().split(',').map(Number)
    switch func
      when 'min' then Math.min params...
      when 'max' then Math.max params...
      when 'sum'
        sum = 0
        for num in params
          sum += num
  ...
  for command in args
    ...
    while command.indexOf('  ') isnt -1
      command = command.replace '  ', ' '
    if command.indexOf('def ') is 0
      if command.indexOf(' ', 4) isnt -1
...
test2 = ["def func sum[1, 2, 3, -6]",
         "def func2 min[1, 2, 3, -6]",
         "def func3 max[1, 2, 3, -6]",
         "def func4 avg[1, 0, 2, -6]",
         "def newList [func, 10, 1]",
         "def newFunc sum[func, 100, newList]",
         "  []  "
]

test3 = ["def var1[1, 2, 3 ,4]",
         "def var2 sum[var1, 20, 70]",
         "def var3 min[var1]",
         "avg[var2, var3]"
]

test4 = ["def var1 [1, 2, 6, 8] ",
